...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        file_path = '/app/card_gorilla_cleaned.csv'
            ...
            with open(file_path, 'r', encoding='utf-8-sig') as f:
                ...
                for row in reader:
                    ...
                    cards_to_create.append(Card(
                        card_name=row['카드명'],
                        company=row['카드사'],
                        annual_fee_domestic=clean_int(row['연회비_국내']),
                        annual_fee_overseas=clean_int(row['연회비_해외']),  # 수정됨
                ...
                if cards_to_create:
                    ...
                    self.stdout.write(self.style.SUCCESS(f'성공: {len(cards_to_create)}개의 카드가 저장되었습니다!'))
                else:
