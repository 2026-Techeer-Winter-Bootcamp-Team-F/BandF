        ...
        try:
            ...
            for bill in billing_items:
                ...
                for history in history_list:
                    ...
                    # 못 찾으면 사용자의 아무 카드나 하나 가져오거나 (위험), 
                    # 없으면 Skip해야함 (FK Constraint).
                    if not user_card:
                         user_card = UserCard.objects.filter(user=request.user).first()
                    
                    if not user_card:
                        # 사용자가 등록한 카드가 아예 없으면 저장 불가
                        logger.warning(f"User {request.user.email} has no cards. Skipping expense save.")
                        continue

                    # Expense 생성/업데이트 (중복 방지: 날짜, 금액, 승인번호 등 조합.. 없는 경우 좀 애매함)
                    # 여기선 일단 무조건 생성 (Create) - 중복 체크 로직이 명확치 않음.
                    # 사용자 편의를 위해 update_or_create를 쓰려면 unique key가 필요.
                    # 승인번호(resApprovalNo)가 있다면 좋은데 예시에 빈값임.
                    # 따라서 날짜 + 가맹점명 + 금액 + 원금 정도로 중복 체크
                    
                    Expense.objects.get_or_create(
                        user=request.user,
                        spent_at=spent_at,
                        merchant_name=history.get('resMemberStoreName', 'Unknown'),
                        amount=int(history.get('resUsedAmount') or 0),
                        defaults={
                            ...
                            'installment_month': int(history.get('resInstallmentMonth') or 0),
                            'round_no': history.get('resRoundNo'),
                            'payment_principal': int(history.get('resPaymentPrincipal') or 0),
                            'fee': int(history.get('resFee') or 0),
                            'payment_amt': int(history.get('resPaymentAmt') or 0),
                            'after_payment_balance': int(history.get('resAfterPaymentBalance') or 0),
                            'earn_point': int(history.get('resEarnPoint') or 0),
                        }
                    )
                    saved_count += 1
