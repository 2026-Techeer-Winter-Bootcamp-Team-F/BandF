...
class Expense(models.Model):
    ...
    # [추가] Codef 청구 내역 상세 필드
    payment_type = models.CharField(max_length=10, null=True, blank=True)  # [설명] 결제 형태 (1: 일시불, etc)
    installment_month = models.IntegerField(default=0)  # [설명] 할부 개월 수
    round_no = models.CharField(max_length=20, null=True, blank=True)  # [설명] 회차 정보
    payment_principal = models.IntegerField(default=0)  # [설명] 원금
    fee = models.IntegerField(default=0)  # [설명] 수수료/이자
    payment_amt = models.IntegerField(null=True, blank=True)  # [설명] 청구 금액
    after_payment_balance = models.IntegerField(default=0)  # [설명] 결제 후 잔액
    earn_point = models.IntegerField(default=0)  # [설명] 적립 포인트
    approval_number = models.CharField(max_length=50, null=True, blank=True)  # [설명] 승인 번호 (중복 방지용)
